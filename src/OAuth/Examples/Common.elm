module OAuth.Examples.Common exposing (Model, OAuthConfiguration,  attrLogo, configurationFor, errorResponseToString, makeInitModel, makeState, oauthProviderFromState, oauthProviderFromString, oauthProviderToString, queryAsFragment, randomBytesFromState, stringDropLeftUntil, stringLeftUntil, view, viewBody, viewError, viewFetching, viewLogin, viewProfile, viewSignInButton)
...
view : String -> { onSignOut : msg, buttons : List (Html msg), sideNote : List (Html msg) } -> Model -> Document msg
view title { onSignOut, buttons, sideNote } model =
    let
        content =
            case ( model.token, model.profile ) of
                ( Nothing, Nothing ) ->
                    viewLogin { buttons = buttons, sideNote = sideNote }
...
viewLogin : { buttons : List (Html msg), sideNote : List (Html msg) } -> List (Html msg)
viewLogin { buttons, sideNote } =
    [ div
        [ style "display" "flex"
...
configurationFor provider =
    ...
    in
    case provider of
        Google -> googleProvider defaultHttpsUrl
